import { getTopInterractedTags } from "@/lib/actions/tag.action";
...
const UserCard = async ({ user }: Props) => {
  const interactedTags = await getTopInterractedTags({ userId: user._id });
        ...
        <div className="mt-5">
          {interactedTags.tags.length > 0 ? (
            <div className="flex items-center gap-2">
              {interactedTags.tags.slice(0, 2).map((tag) => (
